  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => router.push({ pathname: '/support', params: { orderId: order.id.slice(-6).toUpperCase() } })}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          {order.riderName ? (
            <View style={styles.riderCard}>
              ...
              <View style={styles.riderActions}>
                <TouchableOpacity style={styles.riderBtn} onPress={() => showAlert('Call Rider', `${order.riderName || 'Your rider'} will receive a callback request for this order.`)}>
                  <MaterialIcons name="phone" size={20} color={Colors.primary} />
                </TouchableOpacity>
                <TouchableOpacity style={styles.riderBtn} onPress={() => showAlert('Chat Rider', `Message sent to ${order.riderName || 'your rider'}: Please confirm your arrival time.`)}>
        ...
        <View style={styles.itemsCard}>
          ...
          {order.items.map((item, i) => (
            <View key={`${order.id}-${item.restaurantId}-${item.menuItem.id}-${i}`} style={styles.itemRow}>
