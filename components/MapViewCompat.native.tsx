...
export function MapView({ style, initialRegion, children }: MapViewProps) {
  ...
  const sendUpdate = () => webViewRef.current?.postMessage(JSON.stringify({ type: 'REDRUSH_MAP_UPDATE', payload }));

  useEffect(() => {
    webViewRef.current?.postMessage(JSON.stringify({ type: 'REDRUSH_MAP_UPDATE', payload: JSON.parse(payloadJson) }));
  }, [payloadJson]);

  return (
    <View style={[styles.wrapper, style]}>
      <WebView ref={webViewRef} source={{ html: initialHtml }} originWhitelist={['*']} javaScriptEnabled domStorageEnabled startInLoadingState onLoadEnd={sendUpdate} style={styles.webView} />
...
const styles = createThemedStyles(() => ({
  wrapper: { backgroundColor: Colors.surfaceElevated, overflow: 'hidden', position: 'relative' },
  webView: { backgroundColor: Colors.surfaceElevated, flex: 1 },
  badge: { position: 'absolute', bottom: 8, right: 8, backgroundColor: Colors.primary, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4 },
